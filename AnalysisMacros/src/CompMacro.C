...
void CompMacro() {
 ...
 TFile *mpf_new = new TFile("/nfs/dust/cms/user/multh/JEC/2016Legacy/Residuals/Summer16_07Aug2017_V4/AK4CHS/MC_NoReWeighted_CHS_NoEtaCleaning/RunBCDEFGH/plots/pT_extrapolation_mpf.root");
 TFile *pt_new  = new TFile("/nfs/dust/cms/user/multh/JEC/2016Legacy/Residuals/Summer16_07Aug2017_V4/AK4CHS/MC_NoReWeighted_CHS_NoEtaCleaning/RunBCDEFGH/plots/pT_extrapolation_pt.root");
 TFile *mpf_old = new TFile("/nfs/dust/cms/user/multh/JEC/2016ReReco/Residuals/Summer16_03Feb2017_V3/AK4CHS/MC_Reweighted_chsMET_NewSF/RunBCDEFGH/plots/pT_extrapolation_mpf.root");
 TFile *pt_old  = new TFile("/nfs/dust/cms/user/multh/JEC/2016ReReco/Residuals/Summer16_03Feb2017_V3/AK4CHS/MC_Reweighted_chsMET_NewSF/RunBCDEFGH/plots/pT_extrapolation_pt.root");
...
TCanvas *c1 = new TCanvas("c1","c1",600,500);
   ...
   h_pt_extrapolation_mpf_new[i]    = (TH1F*)pt_old->Get("pTextrapolation_Pt_pythia8_pT_"+eta_range2[i]+"_"+eta_range2[i+1]);
...
cout<<"Draw Second Hist"<<endl;
 ...
 if(i==13){ TLegend* leg1 = new TLegend(0.12,0.17,0.47,0.3);
  leg1 -> AddEntry( h_pt_extrapolation_mpf_new[i] ,"Pt balance ","LP");  
  leg1 -> AddEntry( h_pt_extrapolation_mpf_old[i] ,"MPF","LP"); 
 ...
 TLatex *tex = new TLatex();
    ...
    TString text = eta_range2[i] + " < |#eta| < " + eta_range2[i+1];
    tex->DrawLatex(0.52,0.85, text);
 
  c1->SaveAs("pTextrapolation_old_eta_"+ eta_range2[i] + "_" + eta_range2[i+1] + ".pdf");
cout<<"Save"<<endl;
 }
...
TCanvas *c1 = new TCanvas("c1","c1",600,500);
   ...
   h_pt_extrapolation_pT_old[i]    = (TH1F*)mpf_new->Get("pTextrapolation_MPF_pythia8_pT_"+eta_range2[i]+"_"+eta_range2[i+1]);
...
cout<<"Draw Second Hist"<<endl;
 ...
 if(i==13){ TLegend* leg1 = new TLegend(0.12,0.17,0.47,0.3);
  leg1 -> AddEntry( h_pt_extrapolation_pT_new[i] ,"Pt balance","LP");  
  leg1 -> AddEntry( h_pt_extrapolation_pT_old[i] ,"MPF","LP"); 
  leg1->SetTextSize(0.05);
  leg1->SetBorderSize(0);
  leg1->SetFillStyle(0);
 ...
 TLatex *tex = new TLatex();
  ...
  c1->SaveAs("pTextrapolation_new_eta_"+ eta_range2[i] + "_" + eta_range2[i+1] + ".pdf");
cout<<"Save"<<endl;
 }
