...
void CorrectionObject::L2ResOutput_eta(){
 ...
 TCanvas *c10 = new TCanvas();
  ...
  res_logpt_mpf_kfsrfit_val->Draw("E1 SAME");
  for(int i=0;i<4;i++){
    res_logpt_mpf_kfsrval_var[i]->SetLineColor(kRed-3*i);
    res_logpt_mpf_kfsrval_var[i]->SetMarkerColor(kRed-3*i);
  ...
  res_logpt_dijet_kfsrfit_val->SetLineColor(kBlack);
  res_logpt_dijet_kfsrfit_val->Draw("E1 SAME");
  for(int i=0;i<4;i++){
    res_logpt_dijet_kfsrval_var[i]->SetLineColor(kBlue+3*i);
    res_logpt_dijet_kfsrval_var[i]->SetMarkerColor(kBlue+3*i);
    res_logpt_dijet_kfsrval_var[i]->SetMarkerStyle(20+i);
 ...
 TCanvas *c6 = new TCanvas();
  ...
  for(int i=0; i<4; i++){
    ...
    delete res_logpt_dijet_kfsrval_var[i];
