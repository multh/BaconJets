...
void CorrectionObject::kFSR_CorrectFormulae_eta(){
  ...
  //get MC response
  double rel_r_mc[n_pt-1][n_eta_control-1][n_alpha];
  double err_rel_r_mc[n_pt-1][n_eta_control-1][n_alpha];

  double mpf_r_mc[n_pt-1][n_eta_control-1][n_alpha];
  double err_mpf_r_mc[n_pt-1][n_eta_control-1][n_alpha];

  //get DATA response
  double rel_r_data[n_pt-1][n_eta_control-1][n_alpha];
  double err_rel_r_data[n_pt-1][n_eta_control-1][n_alpha];

  double mpf_r_data[n_pt-1][n_eta_control-1][n_alpha];
  double err_mpf_r_data[n_pt-1][n_eta_control-1][n_alpha];


  // get ratio for MC to DATA responses
  double ratio_al_rel_r[n_pt-1][n_eta_control-1][n_alpha]; //ratio at pt,eta,alpha bins
  double err_ratio_al_rel_r[n_pt-1][n_eta_control-1][n_alpha]; //error of ratio at pt,eta,alpha bins

  double ratio_al_mpf_r[n_pt-1][n_eta_control-1][n_alpha]; //ratio at pt,eta,alpha bins
  double err_ratio_al_mpf_r[n_pt-1][n_eta_control-1][n_alpha]; //error of ratio at pt,eta,alpha bins
  ...
  int n_entries_mc[n_eta_control-1][n_alpha][n_pt-1];
  int n_entries_data[n_eta_control-1][n_alpha][n_pt-1];
  int count = 0;
  ...
  for(int i=0; i<n_alpha; i++){
	...
	err_ratio_al_mpf_r[k][j][i] = 0;
      ...
      hdata_asymmetry[j][i] = new TH2D(name,"A in DATA; #bar{p}_{T} [GeV]; A",n_pt-1,pt_bins,nResponseBins, -1.2, 1.2);
      name = name2;name+=count;
      hdata_B[j][i]         = new TH2D(name,"B in DATA; #bar{p}_{T} [GeV];B",n_pt-1,pt_bins,nResponseBins, -1.2, 1.2);
      name = name3; name+=count;
      hmc_asymmetry[j][i]   = new TH2D(name,"A in MC; #bar{p}_{T} [GeV];A",n_pt-1,pt_bins,nResponseBins, -1.2, 1.2);
      name = name4; name+=count;
      hmc_B[j][i]           = new TH2D(name,"B in MC; #bar{p}_{T} [GeV];B",n_pt-1,pt_bins,nResponseBins, -1.2, 1.2);
  ...
  //1) find bin with alpha = alpha_cut: bin no. al_ref
   ...
   TGraphErrors *graph_rel_r_mc[n_pt-1][n_eta_control-1];  //set of points vs alpha
   TGraphErrors *graph_mpf_r_mc[n_pt-1][n_eta_control-1];  //set of points vs alpha

   TGraphErrors *graph_rel_r_data[n_pt-1][n_eta_control-1];  //set of points vs alpha
   TGraphErrors *graph_mpf_r_data[n_pt-1][n_eta_control-1];  //set of points vs alpha

   // Build the Multigraphs containing the ratio of responses (MC/DATA) as a function of alpha
   TGraphErrors *graph_rel_r[n_pt-1][n_eta_control-1];  //set of points vs alpha
   TMultiGraph *pTgraph_rel_r[n_eta_control-1];         //set of different pT bins in on eta bin
   TGraphErrors *graph_mpf_r[n_pt-1][n_eta_control-1];  //set of points vs alpha
   TMultiGraph *pTgraph_mpf_r[n_eta_control-1];         //set of different pT bins in on eta bin


   //Define legend
   TLegend *leg1;
   leg1 = new TLegend(0.28,0.68,0.85,0.89,"","brNDC");//x+0.1
 ...
 //dummy for tdrCanvas
  ...
  h->SetMinimum(0.8);
   ...
   TCanvas* c_rel_r[n_eta_control-1][n_pt-1];
   TCanvas* c_mpf_r[n_eta_control-1][n_pt-1];
   TString name_rel_r[n_eta_control-1][n_pt-1];
   TString name_mpf_r[n_eta_control-1][n_pt-1];
   ...
   for(int j=0; j<n_eta_control-1; j++){
     for(int k=0; k<n_pt-1; k++){
       ...
       name_rel_r[j][k]="rel_r_alpha_"+eta_range_control[j]+"_"+eta_range_control[j+1]+"_pT_"+pt_range[k]+"_"+pt_range[k+1];
       ...
       leg_rel->SetHeader("Rel response, "+eta_range_control[j]+"#leq|#eta|<"+eta_range_control[j+1]+", "+pt_range[k]+"#leq p_{T}<"+pt_range[k+1]);
       ...
       c_rel_r[j][k]->SaveAs(CorrectionObject::_outpath+"plots/control/Rel_ResponseVsAlpha_"+CorrectionObject::_generator_tag+"_eta_"+eta_range2_control[j]+"_"+eta_range2_control[j+1]+"_pT_"+pt_range[k]+"_"+pt_range[k+1]+".pdf");
       ...
       leg_mpf->SetHeader("MPF response, "+eta_range_control[j]+"#leq|#eta|<"+eta_range_control[j+1]+", "+pt_range[k]+"#leq p_{T}<"+pt_range[k+1]);
       ...
       c_mpf_r[j][k]->SaveAs(CorrectionObject::_outpath+"plots/control/MPF_ResponseVsAlpha_"+CorrectionObject::_generator_tag+"_eta_"+eta_range2_control[j]+"_"+eta_range2_control[j+1]+"_pT_"+pt_range[k]+"_"+pt_range[k+1]+".pdf");
  ...
  //*****************************************************************************************************************************************************
   ...
   for (int j=0; j<n_eta_control-1; j++){ 
     ...
     a[j] = new TCanvas(plotname[j], plotname[j], 850,700);
     m_gStyle->SetOptTitle(0);
     ...
     if(!multigraph_rel_empty[j]){
       ...
       pTgraph_rel_r[j]->GetYaxis()->SetTitleSize(0.045);
       pTgraph_rel_r[j]->GetYaxis()->SetTitleOffset(1.);
       pTgraph_rel_r[j]->GetXaxis()->SetTitle("cut on #alpha");
       pTgraph_rel_r[j]->GetXaxis()->SetTitleSize(0.045);
     ...
     tex->DrawLatex(0.5,0.91,CorrectionObject::_lumitag);
     ...
     //cout << "Printing kFSR plots to " << CorrectionObject::_outpath+"plots/kFSR_Pt_"+CorrectionObject::_generator_tag+"_eta_"+eta_range2[j]+".pdf" << endl;
     a[j]->Print(CorrectionObject::_outpath+"plots/kFSR_Pt_"+CorrectionObject::_generator_tag+"_eta_"+eta_range2_control[j]+"_"+eta_range2_control[j+1]+".pdf");
   ...
   TString plotname1[n_eta_control-1];
    for (int j=0; j<n_eta_control-1; j++){
     ...
     b[j] = new TCanvas(plotname1[j], plotname1[j], 850,700);
     m_gStyle->SetOptTitle(0);
     ...
     if(!multigraph_mpf_empty[j]){
       ...
       pTgraph_mpf_r[j]->GetYaxis()->SetTitleSize(0.045);
       pTgraph_mpf_r[j]->GetYaxis()->SetTitleOffset(1.);
       pTgraph_mpf_r[j]->GetXaxis()->SetTitle("cut on #alpha");
       pTgraph_mpf_r[j]->GetXaxis()->SetTitleSize(0.045);
     }
     ...
     b[j]->Print(CorrectionObject::_outpath + "plots/kFSR_MPF_"+CorrectionObject::_generator_tag+"_eta_"+eta_range2_control[j]+"_"+eta_range2_control[j+1]+".pdf");
