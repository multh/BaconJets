...
void CorrectionObject::kFSR_CorrectFormulae_eta(){
 ...
 //get MC response
  double rel_r_mc[n_pt_-1][n_eta_control-1][n_alpha];
  double err_rel_r_mc[n_pt_-1][n_eta_control-1][n_alpha];

  double mpf_r_mc[n_pt_-1][n_eta_control-1][n_alpha];
  double err_mpf_r_mc[n_pt_-1][n_eta_control-1][n_alpha];

  //get DATA response
  double rel_r_data[n_pt_-1][n_eta_control-1][n_alpha];
  double err_rel_r_data[n_pt_-1][n_eta_control-1][n_alpha];

  double mpf_r_data[n_pt_-1][n_eta_control-1][n_alpha];
  double err_mpf_r_data[n_pt_-1][n_eta_control-1][n_alpha];

// get ratio for MC to DATA responses
  double ratio_al_rel_r[n_pt_-1][n_eta_control-1][n_alpha]; //ratio at pt,eta,alpha bins
  double err_ratio_al_rel_r[n_pt_-1][n_eta_control-1][n_alpha]; //error of ratio at pt,eta,alpha bins
  double ratio_al_mpf_r[n_pt_-1][n_eta_control-1][n_alpha]; //ratio at pt,eta,alpha bins
  double err_ratio_al_mpf_r[n_pt_-1][n_eta_control-1][n_alpha]; //error of ratio at pt,eta,alpha bins
  ...
  TH2D *hdata_asymmetry[n_eta_control-1][n_alpha];
  TH2D *hdata_B[n_eta_control-1][n_alpha];
  TH2D *hmc_asymmetry[n_eta_control-1][n_alpha];
  TH2D *hmc_B[n_eta_control-1][n_alpha];

  int n_entries_mc[n_eta_control-1][n_alpha][n_pt_-1];
  int n_entries_data[n_eta_control-1][n_alpha][n_pt_-1];
  int count = 0;
  ...
  cout << "Finished running over DATA events. Read in a total of " << idx << " events." << endl;
   ...
   bool enough_entries[n_alpha][n_eta_control-1][n_pt-1];
   for(int i=0; i<n_alpha; i++){
     for(int j=0; j<n_eta_control-1; j++){
	 eta_cut_bool = fabs(eta_bins_control[j])>eta_cut;
	 for(int k= 0 ; k <  ( eta_cut_bool ?  n_pt_HF-1 : n_pt-1 ) ; k++ ){  
  ...
  //1) find bin with alpha = alpha_cut: bin no. al_ref
   ...
   TGraphErrors *graph_rel_r_mc[n_pt_-1][n_eta_control-1];  //set of points vs alpha
   TGraphErrors *graph_mpf_r_mc[n_pt_-1][n_eta_control-1];  //set of points vs alpha

   TGraphErrors *graph_rel_r_data[n_pt_-1][n_eta_control-1];  //set of points vs alpha
   TGraphErrors *graph_mpf_r_data[n_pt_-1][n_eta_control-1];  //set of points vs alpha

   // Build the Multigraphs containing the ratio of responses (MC/DATA) as a function of alpha
   TGraphErrors *graph_rel_r[n_pt_-1][n_eta_control-1];  //set of points vs alpha
   TMultiGraph *pTgraph_rel_r[n_eta_control-1];         //set of different pT bins in on eta bin
   TGraphErrors *graph_mpf_r[n_pt_-1][n_eta_control-1];  //set of points vs alpha
   TMultiGraph *pTgraph_mpf_r[n_eta_control-1];         //set of different pT bins in on eta bin
   ...
   TCanvas* c_rel_r[n_eta_control-1][n_pt_-1];
   TCanvas* c_mpf_r[n_eta_control-1][n_pt_-1];
   TString name_rel_r[n_eta_control-1][n_pt_-1];
   ...
   for(int j=0; j<n_eta_control-1; j++){
     for(int k = 1 ; k <  ( eta_cut_bool ?  n_pt_HF-1 : n_pt-1 ) ; k++ ){ 
       ...
       name_rel_r[j][k]="rel_r_alpha_"+eta_range_control[j]+"_"+eta_range_control[j+1]+"_pT_"+(eta_cut_bool?pt_range_HF:pt_range)[k]+"_"+(eta_cut_bool?pt_range_HF:pt_range)[k+1];
       ...
       leg_rel->SetHeader("Rel response, "+eta_range_control[j]+"#leq|#eta|<"+eta_range_control[j+1]+", "+(eta_cut_bool?pt_range_HF:pt_range)[k]+"#leq p_{T}<"+(eta_cut_bool?pt_range_HF:pt_range)[k+1]);
       ...
       c_rel_r[j][k]->SaveAs(CorrectionObject::_outpath+"plots/control/Rel_ResponseVsAlpha_"+CorrectionObject::_generator_tag+"_eta_"+eta_range_control2[j]+"_"+eta_range_control2[j+1]+"_pT_"+(eta_cut_bool?pt_range_HF:pt_range)[k]+"_"+(eta_cut_bool?pt_range_HF:pt_range)[k+1]+".pdf");

       name_mpf_r[j][k]="mpf_r_alpha_"+eta_range_control[j]+"_"+eta_range_control[j+1]+"_pT_"+pt_range[k]+"_"+pt_range[k+1];
       ...
       leg_mpf->SetHeader("MPF response, "+eta_range_control[j]+"#leq|#eta|<"+eta_range_control[j+1]+", "+(eta_cut_bool?pt_range_HF:pt_range)[k]+"#leq p_{T}<"+(eta_cut_bool?pt_range_HF:pt_range)[k+1]);
       ...
       c_mpf_r[j][k]->SaveAs(CorrectionObject::_outpath+"plots/control/MPF_ResponseVsAlpha_"+CorrectionObject::_generator_tag+"_eta_"+eta_range_control2[j]+"_"+eta_range_control2[j+1]+"_pT_"+(eta_cut_bool?pt_range_HF:pt_range)[k]+"_"+(eta_cut_bool?pt_range_HF:pt_range)[k+1]+".pdf");
     } 
   }

   bool multigraph_rel_empty[n_eta_control-1];
   bool multigraph_mpf_empty[n_eta_control-1];
  ...
  for(int i=0; i<n_eta_control-1; i++){   
   ...
   for(int j= 0 ; j <  ( eta_cut_bool ?  n_pt_HF-1 : n_pt-1 ) ; j++ ){
     h_kFSR_pt_eta_rel->AddBin(eta_bins_control[i], (eta_cut_bool?pt_bins_HF:pt_bins)[j], eta_bins_control[i+1], (eta_cut_bool?pt_bins_HF:pt_bins)[j+1] );
   }
  }
  ...
  for(int i=0; i<n_eta_control-1; i++){   
   eta_cut_bool = fabs(eta_bins_control[i])>eta_cut;     
   for(int j= 0 ; j <  ( eta_cut_bool ?  n_pt_HF-1 : n_pt-1 ) ; j++ ){
     h_chi2_kFSR_rel->AddBin(eta_bins_control[i], (eta_cut_bool?pt_bins_HF:pt_bins)[j], eta_bins_control[i+1], (eta_cut_bool?pt_bins_HF:pt_bins)[j+1] );
   }
  }
  
   TCanvas* Rel[n_eta_control-1][n_pt_-1];
   TString plotname_rel[n_eta_control-1][n_pt_-1];

   int bincounter = 1;
   
   TF1 *pol_rel[n_eta_control-1][n_pt_-1];
    for(int i=0; i<n_eta_control-1; i++){
     eta_cut_bool = fabs(eta_bins_control[i])>eta_cut;     
     for(int j= 0 ; j <  ( eta_cut_bool ?  n_pt_HF-1 : n_pt-1 ) ; j++ ){ 
	plotname_rel[i][j]="dijet_kfsr_eta_"+eta_range_control[i]+"_"+eta_range_control[i+1]+"_pT_"+(eta_cut_bool?pt_range_HF:pt_range)[j]+"_"+(eta_cut_bool?pt_range_HF:pt_range)[j+1];
	...
	m_gStyle->SetOptTitle(0);
     ...
     Rel[i][j]->Print(CorrectionObject::_outpath+"plots/kFSR_Pt_eta_"+eta_range_control2[i]+"_"+eta_range_control2[i+1]+"_pT_"+(eta_cut_bool?pt_range_HF:pt_range)[j]+"_"+(eta_cut_bool?pt_range_HF:pt_range)[j+1]+".pdf");
     delete tex2_rel;
     delete tex_rel;
     bincounter++;
       }
 ...
 TCanvas* Pt_dep_rel[n_eta_control-1];
 TString name_rel[n_eta_control-1];
 TH1D* h_kFSR_pt_rel[n_eta_control-1];
 for(int i=0; i<n_eta_control-1; i++){
   name_rel[i]="dijet_kfsr_pt_dep_"+eta_range_control[i]+"_"+eta_range_control[i+1];
   ...
   Pt_dep_rel[i]->Print(CorrectionObject::_outpath+"plots/kFSR_Pt_eta_"+eta_range_control2[i]+"_"+eta_range_control2[i+1]+".pdf");
 }
  ...
  for(int i=0; i<n_eta_control-1; i++){   
   eta_cut_bool = fabs(eta_bins_control[i])>eta_cut;     
   for(int j= 0 ; j <  ( eta_cut_bool ?  n_pt_HF-1 : n_pt-1 ) ; j++ ){
     h_kFSR_pt_eta_mpf->AddBin(eta_bins_control[i], (eta_cut_bool?pt_bins_HF:pt_bins)[j], eta_bins_control[i+1], (eta_cut_bool?pt_bins_HF:pt_bins)[j+1] );
   }
  }
  ...
  for(int i=0; i<n_eta_control-1; i++){   
   eta_cut_bool = fabs(eta_bins_control[i])>eta_cut;     
   for(int j= 0 ; j <  ( eta_cut_bool ?  n_pt_HF-1 : n_pt-1 ) ; j++ ){
     h_chi2_kFSR_mpf->AddBin(eta_bins_control[i], (eta_cut_bool?pt_bins_HF:pt_bins)[j], eta_bins_control[i+1], (eta_cut_bool?pt_bins_HF:pt_bins)[j+1] );
   }
  }


  TString plotname2[n_eta_control-1][n_pt_-1];
  TCanvas* MPF[n_eta_control-1][n_pt_-1];
   TF1 *pol_mpf[n_eta_control-1][n_pt_-1];
   bincounter = 1;
    for(int i=0; i<n_eta_control-1; i++){
   eta_cut_bool = fabs(eta_bins_control[i])>eta_cut;     
   for(int j= 0 ; j <  ( eta_cut_bool ?  n_pt_HF-1 : n_pt-1 ) ; j++ ){
	plotname2[i][j]="MPF_kfsr_eta_"+eta_range_control[i]+"_"+eta_range_control[i+1]+"_pT_"+(eta_cut_bool?pt_range_HF:pt_range)[j]+"_"+(eta_cut_bool?pt_range_HF:pt_range)[j+1];
    ...
    TLatex *tex_mpf = new TLatex();
     ...
     double chi2ndf_kFSR_mpf = -1;
      ...
      MPF[i][j]->Print(CorrectionObject::_outpath+"plots/kFSR_MPF_eta_"+eta_range_control2[i]+"_"+eta_range_control2[i+1]+"_pT_"+(eta_cut_bool?pt_range_HF:pt_range)[j]+"_"+(eta_cut_bool?pt_range_HF:pt_range)[j+1]+".pdf");
 ...
 TCanvas* Pt_dep_mpf[n_eta_control-1];
 TString name_mpf[n_eta_control-1];
 TH1D* h_kFSR_pt_mpf[n_eta_control-1]; 
 for(int i=0; i<n_eta_control-1; i++){
   name_mpf[i]="MPF_kfsr_pt_dep_"+eta_range_control[i]+"_"+eta_range_control[i+1];
   eta_cut_bool = fabs(eta_bins_control[i])>eta_cut;     
   ...
   Pt_dep_mpf[i]->Print(CorrectionObject::_outpath+"plots/kFSR_MPF_eta_"+eta_range_control2[i]+"_"+eta_range_control2[i+1]+".pdf");
 }
 ...
 //**********************************************************************************************
   ...
   kFSR_MPF = new TH1D("kfsr_mpf","kfsr_mpf", n_eta_control-1,eta_bins_control);
   kFSR_DiJet = new TH1D("kfsr_dijet","kfsr_dijet", n_eta_control-1,eta_bins_control);
   plotkfsr = new TH1D("kfsr","kfsr", n_eta_control-1,eta_bins_control);

  // Start with pT-balance
  //*****************************************************************************************************************************************************
   //create plots
   TCanvas* a[n_eta_control-1];
   TString plotname[n_eta_control-1];
   TF1 *pol1[n_eta_control-1];
   for (int j=0; j<n_eta_control-1; j++){ 
     plotname[j]="dijet_kfsr_diffPt_eta_"+eta_range_control[j]+"_"+eta_range_control[j+1];
     ...
     cout << "Printing kFSR plots to " << CorrectionObject::_outpath+"plots/kFSR_Pt_"+CorrectionObject::_generator_tag+"_eta_"+eta_range_control2[j]+"_"+eta_range_control2[j+1]+".pdf" << endl;
     a[j]->Print(CorrectionObject::_outpath+"plots/kFSR_Pt_"+CorrectionObject::_generator_tag+"_eta_"+eta_range_control2[j]+"_"+eta_range_control2[j+1]+".pdf");
   ...
   // And now MPF results
   //create plots
   TCanvas* b[n_eta_control-1];
   TString plotname1[n_eta_control-1];
    for (int j=0; j<n_eta_control-1; j++){
     plotname1[j]="mpf_kfsr_diffPt_eta_"+eta_range_control[j]+"_"+eta_range_control[j+1];
     ...
     //save the plots
     cout << "Saving the MPF plots to " << CorrectionObject::_outpath + "plots/kFSR_MPF_"+CorrectionObject::_generator_tag+"_eta_"+eta_range_control2[j]+"_"+eta_range_control2[j+1]+".pdf" << endl;
     b[j]->Print(CorrectionObject::_outpath + "plots/kFSR_MPF_"+CorrectionObject::_generator_tag+"_eta_"+eta_range_control2[j]+"_"+eta_range_control2[j+1]+".pdf");
   ...
   for (int j=0; j<n_eta_control-1; j++){ //n_eta_control-1
     delete pol1[j];
   }
