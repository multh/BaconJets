...
void CorrectionObject::Pt_Extrapolation_Alternative_CorrectFormulae_eta(bool mpfMethod){
  ...
  TH1D* ptave_mc[n_eta_control-1];

  TH1D* ptave_data_eta = new TH1D("ptave_data_eta", " ;#eta;p_{T}^{ave}", n_eta_control-1, eta_bins_control);
  TH1D* ptave_mc_eta   = new TH1D("ptave_mc_eta", " ;#eta;p_{T}^{ave}", n_eta_control-1, eta_bins_control);
  ...
  double ratio_al_rel_r[n_pt_-1][n_eta_control-1]; //ratio at pt,eta,alpha bins
  double err_ratio_al_rel_r[n_pt_-1][n_eta_control-1]; //error of ratio at pt,eta,alpha bins
  double ratio_al_mpf_r[n_pt_-1][n_eta_control-1]; //ratio at pt,eta,alpha bins
  double err_ratio_al_mpf_r[n_pt_-1][n_eta_control-1]; //error of ratio at pt,eta,alpha bins
  ...
  int n_entries_mc[n_eta_control-1][n_pt_-1];
  int n_entries_data[n_eta_control-1][n_pt_-1];

  TH1D *hdata_ptave[n_pt_-1][n_eta_control-1];//pt-ave in each bin of pT_ave in bins of eta
  ...
  for(int j=0; j<n_eta_control-1; j++){
    eta_cut_bool = eta_bins_control[j]>eta_cut;
  ...
  //BAD NOMENCLATURE
  double ratio_mpf[n_eta_control-1][n_pt_-1];     //ratio at pt,eta bins for alpha = 0.3
  double err_ratio_mpf[n_eta_control-1][n_pt_-1]; //error of ratio at pt,eta bins for alpha = 0.3
  for(int j=0; j<n_eta_control-1; j++){
    eta_cut_bool = fabs(eta_bins_control[j])>eta_cut; 
    for(int k=0; k< ( eta_cut_bool ?  n_pt_HF-1 : n_pt-1 ); k++){
      if(mpfMethod){
	...
	//	std::cout<<"ratio_mpf[j][k] = @"<<eta_range_control[j]<<" "<<ratio_mpf[j][k]<<std::endl;
      }
      else{
  ...
  // double zero[n_pt-1];
  double xbin_tgraph[n_eta_control-1][n_pt_-1];
  double zero[n_eta_control-1][n_pt_-1];
...
for(int j=0; j<n_eta_control-1; j++){
  ...
  for (int j=0; j<n_eta_control-1; j++){
   ...
   //Store plots
   if(mpfMethod){
      graph1_mpf[j]->Write("pTextrapolation_MPF_"+CorrectionObject::_generator_tag+"_pT_"+eta_range_control2[j]+"_"+eta_range_control2[j+1]);
      asd[j]->Print(CorrectionObject::_outpath+"plots/pTextrapolation_MPF_"+CorrectionObject::_generator_tag+"_pT_"+eta_range_control2[j]+"_"+eta_range_control2[j+1]+".pdf");
    }
    else{
      graph1_mpf[j]->Write("pTextrapolation_Pt_"+CorrectionObject::_generator_tag+"_pT_"+eta_range_control2[j]+"_"+eta_range_control2[j+1]);
      asd[j]->Print(CorrectionObject::_outpath+"plots/pTextrapolation_Pt_"+CorrectionObject::_generator_tag+"_pT_"+eta_range_control2[j]+"_"+eta_range_control2[j+1]+".pdf");
    }
    delete tex2;
    delete tex;
    delete leg1;
  ...
  if(mpfMethod){
   ...
   double flat_var[n_eta_control-1];
    ...
    for (int j=0; j<n_eta-1; j++){
      ...
      output_loglin << fixed << std::setprecision(6)  << "   " << eta_range_control[j]<< "  " << eta_range_control[j+1] << "   11   10 6500   30   " 
      ...
      uncerts_loglin << fixed << std::setprecision(6) << " " << eta_range_control[j] << "  " << eta_range_control[j+1] << "  6    30    " 
		     << ptave_data[j]->FindLastBinAbove(100.)*10 << " " 
	...
	output_hybrid << fixed << std::setprecision(6)  << "   " << eta_range_control[j]<< "  " << eta_range_control[j+1] << "   11   10 6500   30   " 
        ...
        uncerts_hybrid << fixed << std::setprecision(6) << " " << eta_range_control[j] << "  " << eta_range_control[j+1] << "  6    30    " 
		     << ptave_data[j]->FindLastBinAbove(100.)*10 << " " 
		     << kfsr_FitVal_mpf->GetBinError(j)/norm_loglin << " "<< Vcov[0][j]<<" "<< Vcov[1][j]<<" "<< Vcov[2][j] << endl; 
      }
	...
	output_hybrid_barrel << fixed << std::setprecision(6)  << "   " << eta_range_control[j]<< "  " << eta_range_control[j+1] << "   11   10 6500   30   " 
        ...
        uncerts_hybrid_barrel << fixed << std::setprecision(6) << " " << eta_range_control[j] << "  " << eta_range_control[j+1] << "  6    30    " 
		     << ptave_data[j]->FindLastBinAbove(100.)*10 << " " 
