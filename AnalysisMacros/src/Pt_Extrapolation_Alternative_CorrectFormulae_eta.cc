...
void CorrectionObject::Pt_Extrapolation_Alternative_CorrectFormulae_eta(bool mpfMethod){
  cout << "--------------- Starting Pt_Extrapolation_eta() ---------------" << endl << endl;
  TStyle* m_gStyle = new TStyle();
  m_gStyle->SetOptFit(000);

  // fill the histos for pt average in bins of eta
  TH1D* ptave_data[n_eta_control-1];
  int countPt = 0;
  ...
  for(int i=0; i<n_eta_control-1; i++){
    ...
    selection+=eta_range_control[i+1];
    selection+=" && probejet_eta >=";
    selection+=eta_range_control[i];
    TString var1 = "pt_ave";
  ...
  double ratio_al_rel_r[n_pt-1][n_eta_control-1]; //ratio at pt,eta,alpha bins
  double err_ratio_al_rel_r[n_pt-1][n_eta_control-1]; //error of ratio at pt,eta,alpha bins
  double ratio_al_mpf_r[n_pt-1][n_eta_control-1]; //ratio at pt,eta,alpha bins
  double err_ratio_al_mpf_r[n_pt-1][n_eta_control-1]; //error of ratio at pt,eta,alpha bins

  TProfile *pr_data_asymmetry[n_eta_control-1];// pT-balance response for data  
  TProfile *pr_data_B[n_eta_control-1];//MPF response for data
  TProfile *pr_mc_asymmetry[n_eta_control-1];// pT-balanse responce for MC  
  TProfile *pr_mc_B[n_eta_control-1];//MPF response for MC

  TH2D *hdata_asymmetry[n_eta_control-1];
  TH2D *hdata_B[n_eta_control-1];
  TH2D *hmc_asymmetry[n_eta_control-1];
  TH2D *hmc_B[n_eta_control-1];

  int n_entries_mc[n_eta_control-1][n_pt-1];
  int n_entries_data[n_eta_control-1][n_pt-1];

  TH1D *hdata_ptave[n_pt-1][n_eta_control-1];//pt-ave in each bin of pT_ave in bins of eta
  ...
  bool enough_entries[n_eta_control-1][n_pt-1];
  for(int j=0; j<n_eta_control-1; j++){
    for(int k=0; k<n_pt-1; k++){
  ...
  double val_rel_mc[n_eta_control-1][n_pt-1]; //value at pt,eta
  double rel_mc_err[n_eta_control-1][n_pt-1]; //error of ratio at pt,eta
  double val_mpf_mc[n_eta_control-1][n_pt-1]; //ratio at pt,eta
  double mpf_mc_err[n_eta_control-1][n_pt-1]; //error of ratio at pt,eta
  double val_rel_data[n_eta_control-1][n_pt-1]; //value at pt,eta
  double rel_data_err[n_eta_control-1][n_pt-1]; //error of ratio at pt,eta
  double val_mpf_data[n_eta_control-1][n_pt-1]; //ratio at pt,eta
  double mpf_data_err[n_eta_control-1][n_pt-1]; //error of ratio at pt,eta
  ...
  for(int i=0; i<n_eta_control-1; i++){
    ...
    TGraphErrors *graph_mpf_mc   = new TGraphErrors(n_pt-1, xbin_tgraph, val_mpf_mc[i], zero, mpf_mc_err[i]);
    TGraphErrors *graph_mpf_data = new TGraphErrors(n_pt-1, xbin_tgraph, val_mpf_data[i], zero, mpf_data_err[i]);
    TGraphErrors *graph_rel_mc   = new TGraphErrors(n_pt-1, xbin_tgraph, val_rel_mc[i], zero, rel_mc_err[i]);
    TGraphErrors *graph_rel_data = new TGraphErrors(n_pt-1, xbin_tgraph, val_rel_data[i], zero, rel_data_err[i]);
    ...
    leg_mpf->SetHeader("MPF response, "+eta_range_control[i]+"#leq|#eta|<"+eta_range_control[i+1]+", #alpha<"+s_alpha_cut);
    ...
    c_mpf->SaveAs(CorrectionObject::_outpath+"plots/control/MPF_Response_" + CorrectionObject::_generator_tag + "_eta_" + eta_range2_control[i] + "_" + eta_range2_control[i+1] + ".pdf");
    ...
    leg_rel->SetHeader("p_{T}-balance response, "+eta_range_control[i]+"#leq|#eta|<"+eta_range_control[i+1]); 
    ...
    c_rel->SaveAs(CorrectionObject::_outpath+"plots/control/Rel_Response_" + CorrectionObject::_generator_tag + "_eta_" + eta_range2_control[i] + "_" + eta_range2_control[i+1] + ".pdf");
  ...
  //BAD NOMENCLATURE
  double ratio_mpf[n_eta_control-1][n_pt-1];     //ratio at pt,eta bins for alpha = 0.3
  double err_ratio_mpf[n_eta_control-1][n_pt-1]; //error of ratio at pt,eta bins for alpha = 0.3
  for(int j=0; j<n_eta_control-1; j++){
    for(int k=0; k<n_pt-1; k++){
      if(mpfMethod){
  ...
  // double xbin_tgraph[n_pt-1];
  // double zero[n_pt-1];
  double xbin_tgraph[n_eta_control-1][n_pt-1];
  double zero[n_eta_control-1][n_pt-1];
  for(int i=0;i<n_pt-1;i++){
    for(int j=0; j<n_eta_control-1; j++){
  ...
  TGraphErrors *graph1_mpf[n_eta_control-1];
 
  bool graph_filled[n_eta_control-1];
  for(int j=0; j<n_eta_control-1; j++) graph_filled[j] = false;
  for(int j=0; j<n_eta_control-1; j++){
...
// <<<<<<< HEAD
 ...
 if(graph1_mpf[16]->GetN() == 6){
  ...
  TF1 * f1[n_eta_control-1];
  // create a function for the constant fit
  TF1 * f2[n_eta_control-1];
  ...
  TCanvas* asd[n_eta_control-1];
  TString plotname[n_eta_control-1];
  double Vcov[3][n_eta_control-1];//covariance matrix for log lin fit results
  TH1D* h_chi2_loglin = new TH1D("h_chi2_loglin", "Chi2/ndf for each eta bin;|#eta|;#chi^{2}/ndf", n_eta_control-1, eta_bins_control);
  TH1D* h_chi2_const =  new TH1D("h_chi2_const", "Chi2/ndf for each eta bin;|#eta|;#chi^{2}/ndf", n_eta_control-1, eta_bins_control);
  
  for (int j=0; j<n_eta_control-1; j++){
    if(mpfMethod){
      plotname[j]="mpf_ptextra_"+CorrectionObject::_generator_tag+"_eta_"+eta_range_control[j]+"_"+eta_range_control[j+1];
    }
    else{
      plotname[j]="dijet_ptextra_"+CorrectionObject::_generator_tag+"_eta_"+eta_range_control[j]+"_"+eta_range_control[j+1];
    }
    ...
    TLegend *leg1;
    leg1 = new TLegend(0.2,0.68,0.35,0.88,"","brNDC");//x+0.1
    ...
    leg1->SetTextFont(42);
    if(mpfMethod){
      leg1->SetHeader("MPF #bar{p}_{T} extrapolation, "+eta_range_control[j]+"#leq|#eta|<"+eta_range_control[j+1]);
    }
    else{
      leg1->SetHeader("p_{T} balance #bar{p}_{T} extrapolation, "+eta_range_control[j]+"#leq|#eta|<"+eta_range_control[j+1]);
    }
    ...
    tex->SetNDC();
    tex->SetTextSize(0.045); 
    tex->DrawLatex(0.5,0.96,CorrectionObject::_lumitag+"(13TeV)"); 
   ...
   //Store plots
   if(mpfMethod){
      graph1_mpf[j]->Write("pTextrapolation_MPF_"+CorrectionObject::_generator_tag+"_pT_"+eta_range2_control[j]+"_"+eta_range2_control[j+1]);
       asd[j]->Print(CorrectionObject::_outpath+"plots/pTextrapolation_MPF_"+CorrectionObject::_generator_tag+"_pT_"+eta_range2_control[j]+"_"+eta_range2_control[j+1]+".pdf");
    }
    else{
      graph1_mpf[j]->Write("pTextrapolation_Pt_"+CorrectionObject::_generator_tag+"_pT_"+eta_range2_control[j]+"_"+eta_range2_control[j+1]);
       asd[j]->Print(CorrectionObject::_outpath+"plots/pTextrapolation_Pt_"+CorrectionObject::_generator_tag+"_pT_"+eta_range2_control[j]+"_"+eta_range2_control[j+1]+".pdf");
    }
    delete tex2;
    delete tex;
    delete leg1;
  ...
  if(mpfMethod){
	...
	*/
    ...
    else hist_kfsr_mpf->Fit("kfsr_fit_mpf","SR","",-5.191,5.19);
    ...
    double flat[n_eta_control-1];
    double loglin[n_eta_control-1];

    double flat_var[n_eta_control-1];
    double loglin_var[n_eta_control-1];
    ...
    TH1D* Residual_logpt_MPF = new TH1D("res_logpt_mpf","res_logpt_mpf", n_eta_control-1,eta_bins_control);
    TH1D* Residual_const_MPF = new TH1D("res_const_mpf","res_const_mpf", n_eta_control-1,eta_bins_control);
    TH1D* ptave_const_MPF    = new TH1D("ptave_const_mpf","ptave_const_mpf", n_eta_control-1,eta_bins_control);
    TH1D* ptave_logpt_MPF    = new TH1D("ptave_logpt_mpf","ptave_logpt_mpf", n_eta_control-1,eta_bins_control);

    TH1D* Residual_logpt_MPF_val = new TH1D("res_logpt_mpf_val","res_logpt_mpf_val", n_eta_control-1,eta_bins_control);
    TH1D* Residual_const_MPF_val = new TH1D("res_const_mpf_val","res_const_mpf_val", n_eta_control-1,eta_bins_control);
    ...
    output.open(CorrectionObject::_outpath+"output/Summer16_07Aug2017_MPF_FLAT_L2Residual_"+CorrectionObject::_generator_tag+"_"+CorrectionObject::_jettag+".txt");
    output_loglin.open(CorrectionObject::_outpath+"output/Summer16_07Aug2017_MPF_LOGLIN_L2Residual_"+CorrectionObject::_generator_tag+"_"+CorrectionObject::_jettag+".txt");
    uncerts.open(CorrectionObject::_outpath+"output/Summer16_07Aug2017_MPF_FLAT_L2Residual_"+CorrectionObject::_generator_tag+"_"+CorrectionObject::_jettag+".txt.STAT");
    uncerts_loglin.open(CorrectionObject::_outpath+"output/Summer16_07Aug2017_MPF_LOGLIN_L2Residual_"+CorrectionObject::_generator_tag+"_"+CorrectionObject::_jettag+".txt.STAT");

    output_hybrid.open(CorrectionObject::_outpath+"output/Summer16_07Aug2017_MPF_Hybrid_L2Residual_"+CorrectionObject::_generator_tag+"_"+CorrectionObject::_jettag+".txt");
    uncerts_hybrid.open(CorrectionObject::_outpath+"output/Summer16_07Aug2017_MPF_Hybrid_L2Residual_"+CorrectionObject::_generator_tag+"_"+CorrectionObject::_jettag+".txt.STAT");

    output_hybrid_barrel.open(CorrectionObject::_outpath+"output/Summer16_07Aug2017_MPF_Hybrid_Barrel_L2Residual_"+CorrectionObject::_generator_tag+"_"+CorrectionObject::_jettag+".txt");
    uncerts_hybrid_barrel.open(CorrectionObject::_outpath+"output/Summer16_07Aug2017_MPF_Hybrid_Barrel_L2Residual_"+CorrectionObject::_generator_tag+"_"+CorrectionObject::_jettag+".txt.STAT");

    output  <<"{ 1 JetEta 1 JetPt [2]*([3]*([4]+[5]*TMath::Log(max([0],min([1],x))))*1./([6]+[7]*100./3.*(TMath::Max(0.,1.03091-0.051154*pow(x,-0.154227))-TMath::Max(0.,1.03091-0.051154*TMath::Power(208.,-0.154227)))+[8]*0.021*(-1.+1./(1.+TMath::Exp(-(TMath::Log(x)-5.030)/0.395))))) Correction L2Relative}" << endl;

    output_loglin  << "{ 1 JetEta 1 JetPt [2]*([3]*([4]+[5]*TMath::Log(max([0],min([1],x))))*1./([6]+[7]*100./3.*(TMath::Max(0.,1.03091-0.051154*pow(x,-0.154227))-TMath::Max(0.,1.03091-0.051154*TMath::Power(208.,-0.154227)))+[8]*0.021*(-1.+1./(1.+TMath::Exp(-(TMath::Log(x)-5.030)/0.395))))) Correction L2Relative}" << endl;
    ...
    output_hybrid  << "{ 1 JetEta 1 JetPt [2]*([3]*([4]+[5]*TMath::Log(max([0],min([1],x))))*1./([6]+[7]*100./3.*(TMath::Max(0.,1.03091-0.051154*pow(x,-0.154227))-TMath::Max(0.,1.03091-0.051154*TMath::Power(208.,-0.154227)))+[8]*0.021*(-1.+1./(1.+TMath::Exp(-(TMath::Log(x)-5.030)/0.395))))) Correction L2Relative}" << endl;
    uncerts_hybrid << "{ 1 JetEta 1 JetPt Hybrid Method: Use const fit for 2.65 < |eta| < 3.1, else Log-lin}" << endl;


    cout<<"Setup Barrel output"<<endl;
    output_hybrid_barrel  << "{ 1 JetEta 1 JetPt [2]*([3]*([4]+[5]*TMath::Log(max([0],min([1],x))))*1./([6]+[7]*100./3.*(TMath::Max(0.,1.03091-0.051154*pow(x,-0.154227))-TMath::Max(0.,1.03091-0.051154*TMath::Power(208.,-0.154227)))+[8]*0.021*(-1.+1./(1.+TMath::Exp(-(TMath::Log(x)-5.030)/0.395))))) Correction L2Relative}" << endl;
    uncerts_hybrid_barrel << "{ 1 JetEta 1 JetPt Hybrid Method: Use const fit for 2.65 < |eta| < 3.1, else Log-lin}" << endl;
    cout<<"Created Barrel output"<<endl;
	...
	output_hybrid <<  fixed << std::setprecision(6)  << "  -" << eta_range[j]<< " -" << eta_range[j-1] << "   11   10 6500   30   " << ptave_data[j-1]->FindLastBinAbove(100.)*10 << "   " << 1/loglin_norm_var << " " << hist_kfsr_mpf->GetBinContent(j) << "   " << f1[j-1]->GetParameter(0) << " " << f1[j-1]->GetParameter(1) << "   1 0.0000 0.0" << endl;
      ...
      if(j<6){
	cout<<"Eta Range const j--: -"<<eta_range[j]<<"  -"<<eta_range[j-1]<<endl;
	output_hybrid_barrel << fixed << std::setprecision(6)  << "  -" << eta_range[j]<< " -" << eta_range[j-1] << "   11   10 6500   30   " << ptave_data[j-1]->FindLastBinAbove(100.)*10 << "   " << 1/flat_norm_var << " " << hist_kfsr_mpf->GetBinContent(j) << "   " << f2[j-1]->GetParameter(0) << " 0   1 0.0000 0.0" << endl;
	...
	cout<<"Eta Range: -"<<eta_range[j]<<"  -"<<eta_range[j-1]<<endl;
	output_hybrid_barrel <<  fixed << std::setprecision(6)  << "  -" << eta_range[j]<< " -" << eta_range[j-1] << "   11   10 6500   30   " << ptave_data[j-1]->FindLastBinAbove(100.)*10 << "   " << 1/loglin_norm_var << " " << hist_kfsr_mpf->GetBinContent(j) << "   " << f1[j-1]->GetParameter(0) << " " << f1[j-1]->GetParameter(1) << "   1 0.0000 0.0" << endl;
  ...
  /* ++++++++++++++++++++++++++ Calculate L2Residuals PT balance ++++++++++++++++++++++++++++++ */
	...
	*/
    ...
    TH1D* Residual_logpt_DiJet = new TH1D("res_logpt_dijet","res_logpt_dijet", n_eta_control-1,eta_bins_control);
    TH1D* Residual_const_DiJet = new TH1D("res_const_dijet","res_const_dijet", n_eta_control-1,eta_bins_control);
    TH1D* ptave_const_DiJet    = new TH1D("ptave_const_dijet","ptave_const_dijet", n_eta_control-1,eta_bins_control);
    TH1D* ptave_logpt_DiJet    = new TH1D("ptave_logpt_dijet","ptave_logpt_dijet", n_eta_control-1,eta_bins_control);

    TH1D* Residual_logpt_DiJet_val = new TH1D("res_logpt_dijet_val","res_logpt_dijet_val", n_eta_control-1,eta_bins_control);
    TH1D* Residual_const_DiJet_val = new TH1D("res_const_dijet_val","res_const_dijet_val", n_eta_control-1,eta_bins_control);

    ofstream output, output_loglin, uncerts, uncerts_loglin, output_hybrid, uncerts_hybrid, output_hybrid_barrel, uncerts_hybrid_barrel;
    output.open(CorrectionObject::_outpath+"output/Summer16_07Aug2017_pT_FLAT_L2Residual_"+CorrectionObject::_generator_tag+"_"+CorrectionObject::_jettag+".txt");
    output_loglin.open(CorrectionObject::_outpath+"output/Summer16_07Aug2017_pT_LOGLIN_L2Residual_"+CorrectionObject::_generator_tag+"_"+CorrectionObject::_jettag+".txt");
    uncerts.open(CorrectionObject::_outpath+"output/Summer16_07Aug2017_pT_FLAT_L2Residual_"+CorrectionObject::_generator_tag+"_"+CorrectionObject::_jettag+".txt.STAT");
    uncerts_loglin.open(CorrectionObject::_outpath+"output/Summer16_07Aug2017_pT_LOGLIN_L2Residual_"+CorrectionObject::_generator_tag+"_"+CorrectionObject::_jettag+".txt.STAT");

    output_hybrid.open(CorrectionObject::_outpath+"output/Summer16_07Aug2017_pT_Hybrid_L2Residual_"+CorrectionObject::_generator_tag+"_"+CorrectionObject::_jettag+".txt");
    uncerts_hybrid.open(CorrectionObject::_outpath+"output/Summer16_07Aug2017_pT_Hybrid_L2Residual_"+CorrectionObject::_generator_tag+"_"+CorrectionObject::_jettag+".txt.STAT");

    output_hybrid_barrel.open(CorrectionObject::_outpath+"output/Summer16_07Aug2017_pT_Hybrid_Barrel_L2Residual_"+CorrectionObject::_generator_tag+"_"+CorrectionObject::_jettag+".txt");
    uncerts_hybrid_barrel.open(CorrectionObject::_outpath+"output/Summer16_07Aug2017_pT_Hybrid_Barrel_L2Residual_"+CorrectionObject::_generator_tag+"_"+CorrectionObject::_jettag+".txt.STAT");
	...
	output_hybrid <<  fixed << std::setprecision(6)  << "  -" << eta_range[j]<< " -" << eta_range[j-1] << "   11   10 6500   30   " << ptave_data[j-1]->FindLastBinAbove(100.)*10 << "   " << 1/loglin_norm_var << " " << hist_kfsr_dijet->GetBinContent(j) << "   " << f1[j-1]->GetParameter(0) << " " << f1[j-1]->GetParameter(1) << "   1 0.0000 0.0" << endl;
      ...
      if(j<6){
	cout<<"Eta Range const j--: -"<<eta_range[j]<<"  -"<<eta_range[j-1]<<endl;
	output_hybrid_barrel << fixed << std::setprecision(6)  << "  -" << eta_range[j]<< " -" << eta_range[j-1] << "   11   10 6500   30   " << ptave_data[j-1]->FindLastBinAbove(100.)*10 << "   " << 1/flat_norm_var << " " << hist_kfsr_dijet->GetBinContent(j) << "   " << f2[j-1]->GetParameter(0) << " 0   1 0.0000 0.0" << endl;
	...
	cout<<"Eta Range: -"<<eta_range[j]<<"  -"<<eta_range[j-1]<<endl;
	output_hybrid_barrel <<  fixed << std::setprecision(6)  << "  -" << eta_range[j]<< " -" << eta_range[j-1] << "   11   10 6500   30   " << ptave_data[j-1]->FindLastBinAbove(100.)*10 << "   " << 1/loglin_norm_var << " " << hist_kfsr_dijet->GetBinContent(j) << "   " << f1[j-1]->GetParameter(0) << " " << f1[j-1]->GetParameter(1) << "   1 0.0000 0.0" << endl;
...
//delete everything
  ...
  for(int i=0; i<n_eta_control-1; i++) delete ptave_data[i];
  delete m_gStyle;
