...
void PTextrapolation_TTree(bool mpfMethod, TString path, TFile* datafile, TFile* MCfile, TString txttag, TString jettag, TString variation, TString tag, double al_cut=0.2, int nResponseBins=100){
...
// Create the tree reader and its data containers
  ...
  if(mpfMethod){
    ...
    */
    ...
    uncerts.open(path+"output/Spring16_25ns_MPF_FLAT_L2Residual_"+txttag+"_"+jettag+"_"+variation+".txt.STAT");
    uncerts_loglin.open(path+"output/Spring16_25ns_MPF_LOGLIN_L2Residual_"+txttag+"_"+jettag+"_"+variation+".txt.STAT");
    ...
    TFile* outputfile2;
    if(syst==5){
      outputfile2 = new TFile(path+"Histo_ptave_MPF_L1"+tag+".root","RECREATE");
    }
    if(syst==0){
  ...
  // DIJET balance
    ...
    */
    ...
    uncerts.open(path+"output/Spring16_25ns_pT_FLAT_L2Residual_"+txttag+"_"+jettag+"_"+variation+".txt.STAT");
    uncerts_loglin.open(path+"output/Spring16_25ns_pT_LOGLIN_L2Residual_"+txttag+"_"+jettag+"_"+variation+".txt.STAT");
    output  << "{ 1 JetEta 1 JetPt [2]*([3]*([4]+[5]*TMath::Log(max([0],min([1],x))))*1./([6]+[7]*100./3.*(TMath::Max(0.,1.03091-0.051154*pow(x,-0.154227))-TMath::Max(0.,1.03091-0.051154*TMath::Power(208.,-0.154227)))+[8]*((-2.36997+0.413917*TMath::Log(x))/x-(-2.36997+0.413917*TMath::Log(208))/208))) Correction L2Relative}" << endl;
    output_loglin  << "{ 1 JetEta 1 JetPt [2]*([3]*([4]+[5]*TMath::Log(max([0],min([1],x))))*1./([6]+[7]*100./3.*(TMath::Max(0.,1.03091-0.051154*pow(x,-0.154227))-TMath::Max(0.,1.03091-0.051154*TMath::Power(208.,-0.154227)))+[8]*((-2.36997+0.413917*TMath::Log(x))/x-(-2.36997+0.413917*TMath::Log(208))/208))) Correction L2Relative}" << endl;
    uncerts << "{ 1 JetEta 1 JetPt [0] Correction L2Relative}" << endl;
    uncerts_loglin << "{ 1 JetEta 1 JetPt [0] Correction L2Relative}" << endl;
