...
void kFSR_TTree(bool mpfMethod, TString path, TFile* datafile, TFile* MCfile, double al_cut=0.2,int nResponseBins=100){
  cout<<"alpha cut @"<<al_cut<<endl;
  TStyle* m_gStyle = new TStyle();

  m_gStyle->SetOptFit(0);
  // get ratio for MC to DATA responses
  double ratio_al_rel_r[n_eta-1][n_alpha]; //ratio at pt,eta,alpha bins
  double err_ratio_al_rel_r[n_eta-1][n_alpha]; //error of ratio at pt,eta,alpha bins
  double ratio_al_mpf_r[n_eta-1][n_alpha]; //ratio at pt,eta,alpha bins
  double err_ratio_al_mpf_r[n_eta-1][n_alpha]; //error of ratio at pt,eta,alpha bins
  TH1D *hdata_rel_r[n_eta-1][n_alpha];// pT-balance responce for data
  TH1D *hdata_mpf_r[n_eta-1][n_alpha];//MPF responce for data
  TH1D *hmc_rel_r[n_eta-1][n_alpha];// pT-balance responce for MC
  TH1D *hmc_mpf_r[n_eta-1][n_alpha];//MPF responce for MC
  int count = 0;
  ...
  for(int i=0; i<n_alpha; i++){
	...
	ratio_al_rel_r[j][i] = 0;
	err_ratio_al_rel_r[j][i] = 0;
	ratio_al_mpf_r[j][i] = 0;
	err_ratio_al_mpf_r[j][i] = 0;
	TString name = name1; name+=count;
	hdata_rel_r[j][i] = new TH1D(name,"",nResponseBins, 0, 2.5);
	name = name2;name+=count;
	hdata_mpf_r[j][i] = new TH1D(name,"",nResponseBins, 0, 2.5);
	name = name3; name+=count;
	hmc_rel_r[j][i] = new TH1D(name,"",nResponseBins, 0, 2.5);
	name = name4; name+=count;
	hmc_mpf_r[j][i] = new TH1D(name,"",nResponseBins, 0, 2.5);
	count++;
    }
  }
   ...
   //     myReader_DATA.Next();//TEST
     // cout<<"DATA point: alpha = "<<*alpha_data<<" eta_probe = "<<*probejet_eta_data<<" pT_ave = "<<*pt_ave_data<<endl;
	   ...
	   for(int j=0; j<n_eta-1; j++){
		 ...
		 //		  <<" and pT range = "<<pt_bins<<", "<<pt_bins[k+1]<<std::endl;
		 //	 std::cout<<"probe_eta = "<<*probejet_eta_data<<" pT_ave = "<<*pt_ave_data<<std::endl;
		 //		 cout<<"DATA response "<<*rel_r_data<<endl;
		 hdata_rel_r[j][i]->Fill(*rel_r_data,*weight_data);
		 hdata_mpf_r[j][i]->Fill(*mpf_r_data,*weight_data);
	       }
	   }
     }
   }
   ...
   while (myReader_MC.Next()) {
	 ...
	 if(*alpha_mc>alpha_bins[i]) continue;
	   ...
	   hmc_rel_r[j][i]->Fill(*rel_r_mc,*weight_mc);
	   hmc_mpf_r[j][i]->Fill(*mpf_r_mc,*weight_mc);
	 }
       }
     }
   }
   ...
   for(int i=0; i<n_alpha; i++){
     for(int j=0; j<n_eta-1; j++){
	 double integ_rel_mc = hmc_rel_r[j][i]->Integral();
	 hmc_rel_r[j][i]->Scale(1./integ_rel_mc);
	 double integ_mpf_mc = hmc_mpf_r[j][i]->Integral();
	 hmc_mpf_r[j][i]->Scale(1./integ_mpf_mc);
	 double integ_rel_data = hdata_rel_r[j][i]->Integral();
	 hdata_rel_r[j][i]->Scale(1./integ_rel_data);
	 double integ_mpf_data = hdata_mpf_r[j][i]->Integral();
	 hdata_mpf_r[j][i]->Scale(1./integ_mpf_data);
     }
   }
   ...
   for(int i=0; i<n_alpha; i++){
	...
	ratio_al_rel_r[j][i] = ratio_res_rel_r.first;
	err_ratio_al_rel_r[j][i] = ratio_res_rel_r.second;
	ratio_al_mpf_r[j][i] = ratio_res_mpf_r.first;
	err_ratio_al_mpf_r[j][i] = ratio_res_mpf_r.second;
  ...
  for(int j=0; j<n_eta-1; j++){
      ...
      double norm_al02_rel_r = ratio_al_rel_r[j][al_ref];
      double err_norm_al02_rel_r = err_ratio_al_rel_r[j][al_ref];
      double norm_al02_mpf_r = ratio_al_mpf_r[j][al_ref];
      double err_norm_al02_mpf_r = err_ratio_al_mpf_r[j][al_ref];
      for(int i=0; i<n_alpha; i++){
  	if(norm_al02_rel_r>0){
  	  ratio_al_rel_r[j][i] =   ratio_al_rel_r[j][i]/norm_al02_rel_r;
  	  err_ratio_al_rel_r[j][i] = sqrt(abs(pow(err_ratio_al_rel_r[j][i],2)-pow(err_norm_al02_rel_r,2)));
  	}
  	if(norm_al02_mpf_r>0){
  	  ratio_al_mpf_r[j][i] =   ratio_al_mpf_r[j][i]/norm_al02_mpf_r;
  	  err_ratio_al_mpf_r[j][i] = sqrt(abs(pow(err_ratio_al_mpf_r[j][i],2)-pow(err_norm_al02_mpf_r,2)));
  	}
      }
    }
  ...
  leg1->SetTextFont(42);
  leg1->SetNColumns(2);
  TGraphErrors *graph_rel_r[n_eta-1];//set of points vs alpha
  TMultiGraph *pTgraph_rel_r[n_eta-1];//set of different pT bins in on eta bin
  TGraphErrors *graph_mpf_r[n_eta-1];//set of points vs alpha
  ...
  for(int j=0; j<n_eta-1; j++){
    graph_rel_r[j] = new TGraphErrors(n_alpha,xbin_tgraph,ratio_al_rel_r[j],zero,err_ratio_al_rel_r[j]);
    graph_rel_r[j] = (TGraphErrors*)CleanEmptyPoints(graph_rel_r[j]);
    graph_rel_r[j]->SetMarkerSize(1.5);
    graph_rel_r[j]->SetMarkerStyle(20);
    graph_rel_r[j]->SetMarkerColor(1);
    graph_rel_r[j]->SetLineColor(1);
    // //if(graph_rel_r[j]->GetN()>0) 
    // 	pTgraph_rel_r[j]->Add(graph_rel_r[j]);
    //   TString pTbin_label = "";
    //   pTbin_label+=pt_bins;
    //   pTbin_label+=" < p_{T} < ";
    //   pTbin_label+=pt_bins[k+1];
    //   //      cout<<j<<" "<<k<<" "<<pTbin_label<<endl;
    //   if(j==0) leg1->AddEntry(graph_rel_r[j],pTbin_label,"epl");

    graph_mpf_r[j] = new TGraphErrors(n_alpha,xbin_tgraph,ratio_al_mpf_r[j],zero,err_ratio_al_mpf_r[j]);
    graph_mpf_r[j] = (TGraphErrors*)CleanEmptyPoints(graph_mpf_r[j]);
    graph_mpf_r[j]->SetMarkerSize(1.3);
    graph_mpf_r[j]->SetMarkerStyle(20);
    graph_mpf_r[j]->SetMarkerColor(1);
    graph_mpf_r[j]->SetLineColor(1);

      // if(k<9){
      // graph_mpf_r[j]->SetMarkerColor(k+1);
      // graph_mpf_r[j]->SetLineColor(k+1);
      // }
      // else{
      // 	graph_mpf_r[j]->SetMarkerColor(k+19);
      // 	graph_mpf_r[j]->SetLineColor(k+19);
      // }
      // if(graph_mpf_r[j]->GetN()>0) 
...
//   // //%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
  ...
  for (int j=0; j<n_eta-1; j++){
    ...
    graph_rel_r[j]->Draw("AP");
    graph_rel_r[j]->GetYaxis()->SetRangeUser(0.92,1.08);
    //    graph_rel_r[j]->GetYaxis()->SetRangeUser(0.9,1.1);
    graph_rel_r[j]->GetXaxis()->SetRangeUser(0,alpha_bins[n_alpha-1]+0.01);
    //    graph_rel_r[j]->GetYaxis()->SetTitle("(R_{MC}/R_{DATA})/(R_{MC}/R_{DATA})_{#alpha<0.2}");
    graph_rel_r[j]->SetTitle();
    graph_rel_r[j]->GetYaxis()->SetTitle("(R_{MC}/R_{DATA})");
    graph_rel_r[j]->GetXaxis()->SetTitle("cut on #alpha");
    //        pol1[j] = new TF1("pol1","pol1",0.19,0.41);  //TEST
    //    pol1[j] = new TF1("pol1","pol1",0.04,0.46);  //TEST
    pol1[j] = new TF1("pol1","pol1",0.19,0.36);  //TEST
    //    pol1[j] = new TF1("pol1","pol1",0.39,1.21);//TEST
    //    pol1[j] = new TF1("pol1","pol1",0.39,1.01);//TEST
    //    pol1[j] = new TF1("pol1","pol1",0.04,0.86);  //TEST
    pol1[j]->SetParameters(0,0);
    graph_rel_r[j]->Fit(pol1[j],"R");
    line->SetLineStyle(2);
    line->Draw("SAME");
 ...
 //create plots
  ...
  for (int j=0; j<n_eta-1; j++){
    ...
    m_gStyle->SetOptTitle(0);
    graph_mpf_r[j]->Draw("AP");
    graph_mpf_r[j]->SetTitle("");
    graph_mpf_r[j]->GetYaxis()->SetRangeUser(0.92,1.08);
    //    graph_mpf_r[j]->GetYaxis()->SetRangeUser(0.9,1.1);
    graph_mpf_r[j]->GetXaxis()->SetRangeUser(0,alpha_bins[n_alpha-1]+0.01);
    //    graph_mpf_r[j]->GetYaxis()->SetTitle("(R_{MC}/R_{DATA})/(R_{MC}/R_{DATA})_{#alpha<0.2}");
    graph_mpf_r[j]->GetYaxis()->SetTitle("(R_{MC}/R_{DATA})");
    graph_mpf_r[j]->GetXaxis()->SetTitle("cut on #alpha");
