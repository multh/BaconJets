...
void ResPtDependence(TString path, double al_cut=0.2, TString method = "COMB"){
  ...
  res_nominal->SetLineColor(kGreen+1);
  res_central->SetLineColor(kRed+1);
  res_down->SetLineColor(kBlue+1);
  res_up->SetLineColor(kBlack);
  ...
  res_nominal->GetYaxis()->SetRangeUser(0.9,1.2);
  //  res_nominal->GetYaxis()->SetRangeUser(0.95,1.15);
  res_nominal->Draw("E1");
  ...
  leg1 -> AddEntry(res_nominal, "nominal","L");
  leg1->Draw();
