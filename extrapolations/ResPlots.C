...
void ResPlots(TString path){
  ...
  TH1D* consmpf = resmpf->Get("res_const_mpf");
  TH1D* ptmpf = resmpf->Get("res_logpt_mpf");
  TFile* resdijet = new TFile(path+"Histo_Res_DiJet_L1.root","READ"); 
  TH1D* consdijet = resdijet->Get("res_const_dijet");
  TH1D* ptdijet = resdijet->Get("res_logpt_dijet");
  ...
  c1->SaveAs("L2Res_25ns_2p11fb.pdf");
