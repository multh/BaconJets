...
void VarPtaveDijet(TString path){
  gStyle->SetOptStat(0);
  gStyle->SetOptTitle(0);

  // get the residual root file for MPF and pt balance
  TFile* resmpf = new TFile(path+"Histo_Res_DiJet_L1_central.root","READ");
  TH1D* ptmpf = resmpf->Get("res_logpt_dijet");
  TFile* resmpf_half = new TFile(path+"Histo_Res_DiJet_L1_down.root","READ"); 
  TH1D* ptmpf_half = resmpf_half->Get("res_logpt_dijet");
  TFile* resmpf_twice = new TFile(path+"Histo_Res_DiJet_L1_up.root","READ"); 
  TH1D* ptmpf_twice = resmpf_twice->Get("res_logpt_dijet");
  TFile* resmpf_four = new TFile(path+"Histo_Res_DiJet_L1_doubleup.root","READ"); 
  TH1D* ptmpf_four = resmpf_four->Get("res_logpt_dijet");
  ...
  c1->SaveAs("L2Res_25ns_2p11fb_fixedPtave_ptbalance.pdf");
