...
void kFSR(bool mpfMethod, TString path, TFile* datafile, TFile* MCfile){
  ...
  for(int j=0; j<n_eta-1; j++){
    for(int i=0; i<n_alpha-1; i++){
      ...
      ratio[j]->SetBinContent(i+1, (mc[i][j]->GetMean() / data[i][j]->GetMean()) / (mc[3][j]->GetMean() / data[3][j]->GetMean()) );
  ...
  //  double xbin_tgraph[n_alpha-1] = {0.1, 0.15, 0.2, 0.0.3,0.4};
  ...
  for(int j=0; j<n_eta-1; j++){
    ...
    //    graph1[j] = new TGraphErrors(n_alpha-1, alpha_bins, content , zero, error);
    // graph1[j]->Print();
  }
